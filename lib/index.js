...
(function() {
	function Courier() {
		...
		this.usps = function(tracking_number, callback) {
			...
			var uspsUserID = '754REDBI4747';

			// Construct string for Xml request in Usps Api
			var strXmlApiRequest = '<TrackFieldRequest USERID=\"' + uspsUserID + '\"><TrackID ID=\"' + tracking_number + '\"></TrackID></TrackFieldRequest>';
		...
		this._getResultFromHkpostBody = function(hkpostBody) {
			...
			$('table.detail').last().find('tr').each(function(index) {
				if (index != 0) {
					...
					// Extract Country Initial
					var arrCountry = strCountry.split(' ');
					var strCountryInitialOnly = '';
					arrCountry.forEach(function(element, index, array) {
						strCountryInitialOnly += element.charAt(0);
					});

					var checkpointsObject = {
						country_name: strCountryInitialOnly,
						message: strStatus,
